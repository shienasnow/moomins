...
class ShotLoader (QWidget): 
    ...
    def get_task_data(self): # 할당된 데이터 정보 가져와서 딕셔너리로 정리
        ...
        for data_dict in datas_list: # 리스트 풀기 
                ...
                if project_name == selected_project:
                    for seq in seq_data_dict.keys():
                        ...
                        if seq_name_parts == selected_seq_name_parts:
                            ...
                            # print (project_name, matching_seq_dict)
    ...
    def make_table_hard_coding(self, row, data_dict, seq_name): # 하드 코딩으로 테이블 만들기
        ...
        label_node_name5.setAlignment(Qt.AlignCenter | Qt.AlignVCenter)
        label_node_name5.setStyleSheet("font-size: 12px;")
        label_node_name5.setFixedSize(160, 20)
        ...
        label_node_name6 = QLabel()
        if status == "wtg":
            image_path = "/home/rapa/pipeline_0825/pipeline/sourceimages/wtg.png"
        elif status == "re":
            image_path = "/home/rapa/pipeline_0825/pipeline/sourceimages/re.png"
        elif status == "wip":
            image_path = "/home/rapa/pipeline_0825/pipeline/sourceimages/wip.png"
        elif status == "pub":
            image_path = "/home/rapa/pipeline_0825/pipeline/sourceimages/pub.png"
        elif status == "sc":
            image_path = "/home/rapa/pipeline_0825/pipeline/sourceimages/sc.png"
        elif status == "fin":
            image_path = "/home/rapa/pipeline_0825/pipeline/sourceimages/fin.png"

        pixmap = QPixmap(image_path)
        scaled_pixmap = pixmap.scaled(30, 20)
    ...
    def get_click_data(self, row): # 테이블위젯에서 선택한 row 를 전역변수로 선언해주는 메서드
        """
   ...
   # listwidget 관련 메서드            
    ...
    def double_clicked_item(self,item): # 리스트 위젯에서 파일 더블클릭하면 실행하기
        ...
        """
        _, task, _, version, wip_path, _, _ = self.get_tablewidget_data()
        ...
        # 작업 테스크 별 오픈되는 프로그램 분리
        if task in ["ly", "ani"]:
            command = ["maya", "-file", file_path]
            subprocess.Popen(command)
        elif task == "lgt":
            if file_name.split(".")[-1] == "mb":
                command = ["maya", "-file", file_path]
                subprocess.Popen(command)
            else:
    ...
    def make_new_scene(self): # New Scene 눌렀을 때 파트별로 새 씬(w001)이 열리거나 이미 있다면 작업 중이라고 경고 창
        ...
        seq_name, task, status, version, wip_path, _, ver_path = self.get_tablewidget_data()
        ...
        if task in ["ly", "ani"]: 
            if not os.path.exists(f"{ver_path}/{seq_name}_{version}_w001.mb"):
                ...
                subprocess.Popen(["maya", "-file", f"{ver_path}/{seq_name}_{version}_w001.mb"]) # 파일 실행
                return
            self.msg_box("WipStatusError")

        # 라이팅 작업자의 경우 wip 파일 없으면 마야가 만들어지고 실행, status 가 sc 일 경우 누크가 뉴씬 되도록 
        if task == "lgt":
            if os.path.exists(f"{ver_path}/{seq_name}_{version}_w001.mb"): # 마야 경로가 존재할 경우
                if status == "sc": # 프리컴프 단계에선 누크가 뉴씬 되도록
    ...
    def version_up_for_retake(self): # pub 한 파일 retake 받았을 때 wip 에 새로운 ver 폴더와 wip 파일 생성되는 메서드
        ...
        seq_name, task, status, _, wip_path, pub_path = self.get_tablewidget_data()
        ...
        if status == "re":
            ...
            if task in ["ly", "ani", "lgt"]:
                subprocess.Popen(["maya", "-file", f"{new_ver_path}/{seq_name}_{new_ver_num}_w001.mb"])
            elif task == "fx":
                self.mX
            elif task == "cmp":

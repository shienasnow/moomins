...
class AssetLoader(QWidget):
    ...
    def __init__(self, user_id):
        ...
        self.assign_datas = self.sg_cls.get_assgined_user_project_by_task(user_id)
    ...
    def find_file_version(self,project,asset_name,asset_type,task,version):    # 파일버전을 찾는 함수.
    ...
    def import_new_scene(self): # 파일이 없으면 새로운 마야 파일을 열어주는 시스템 ################# status update
        ...
        self.base_path = path.replace(f"/scenes/{version}","")            # 파일을 생성할때는 폴더가 생성되어있지 않기때문에 scene을 지워서 다시 만들어준다.
        ...
        for common_item in common_list:
            create_path = f"{self.base_path}/{common_item}/{version}"
    ...
    def run_maya(self,add_file_path):
        process = subprocess.Popen(["/bin/bash", "-i", "-c", f'maya {add_file_path}'],start_new_session=True)
        try:
            ...
            if process.poll() is None:
                print("Maya did not exit gracefully, attempting to kill the process.")
                process.terminate()
                process.wait()
                if process.poll() is None:
                    print("Forced termination of Maya.")
                    process.kill()
                    process.wait()
    ...
    def make_version_up_for_retake(self): # retake에서 status를 확인후 버전업되는 함수 ################# status update
        ...
        self.sg_cls.sg_status_update(asset_name,task) ############################################################ status update
        self.reload_data()
        self.import_ini_file()
    ...
    def reload_data(self):
        self.assign_datas = {}
        self.sg_cls.get_assgined_user_project_by_task()
    ...
    def find_image_icon(self,file_type):    # file_type을 받아서 이미지path를 받아오는 함수
        ...
        sourceimages_ini_file_path = os.path.join(path,"sourceimages.ini")
        config = ConfigParser()
        config.optionxform = str
        ...
        image_path_temp = config["icons"][icon_name]                # f"{path}/sourceimages/icon_name"  
        image_path = image_path_temp.format(path=path)              # {path}를 현재 경로로 대체한다.
        
        return image_path
    
    
    
    def make_table_by_field(self,project,field,row_idx): # 하드코딩한 함수...ㅅㅂㅅㅂㅅㅂㅅㅂㅅㅂ
        """
        하드 코딩으로 ui 만들기...!
        """

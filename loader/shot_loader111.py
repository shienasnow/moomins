...
class ShotLoader (QWidget): 
    ...
    def click_event(self, item, column): # 트리 위젯에서 클릭했을 때 매치 되는 데이터 보내기
        ...
        if item.childCount() > 0: # 프로젝트 ( 부모 )
            selected_project = item.text(column)
            for project_name, seq_data in self.projects.items():
                if project_name == selected_project:
                        matching_project = self.projects[selected_project]
                        if matching_project:
                            self.update_table_items(matching_project, None)

        else:                      # 시퀀스 ( 자식 )
            matching_seq = {}
            selected_seq_name_parts = item.text(column)
            selected_project = item.parent().text(column)
            for project_name, seq_data in self.projects.items():
                if project_name == selected_project:
                    for seq in seq_data.keys():
                        seq_name_parts = seq.split("_")[0]
                        if seq_name_parts == selected_seq_name_parts:
                            matching_seq_data = seq_data[seq]
                            matching_seq[seq] = matching_seq_data
                            if matching_seq:
                                self.update_table_items(None, matching_seq)
                ...
                if project == selected_project:
                    for seq, data in seqs.items():
                        ...
                        if table_seq == seq:
                            ...
                            return seq, part, status, version, wip_path, pub_path
                        
                        continue
            else:
                selected_project = item.parent().text(0) # 트리에서 선택한 아이템의 부모 ( 선택한 시퀀스의 프로젝트 이름 )
                if project == selected_project:
    ...
    def make_new_scene(self): # New Scene 눌렀을 때 파트별로 새 씬(w001)이 열리거나 이미 있다면 작업 중이라고 경고 창
        ...
        seq, part, _, version, wip_path, _ = self.get_tablewidget_data()
        ...
        if maya_user:
            if not os.path.exists(f"{ver_path}/{seq}_{version}_w001.mb"):
                ...
                subprocess.Popen(["maya", "-file", f"{ver_path}/{seq}_{version}_w001.mb"]) # 파일 실행
                return
    ...
    def make_new_wip_version(self): # wip 파일 버전 업 / wip 파일이 아예 없다면 경고창 띄우기
        ...
        _, part, _, version, wip_path, _ = self.get_tablewidget_data()
    ...
    def open_current_path(self): # 현재 버전의 경로를 열어주는 메서드
        ...
        _, _, _, version, wip_path, _ = self.get_tablewidget_data()
    ...
    def version_up_for_retake(self): # pub 한 파일 retake 받았을 때 wip 에 새로운 ver 폴더와 wip 파일 생성되는 메서드
        ...
        seq, part, status, _, wip_path, pub_path = self.get_tablewidget_data()

        scene_path = f"{wip_path}/scene"
        ...
        if status == "re":
            ...
            if maya_user:
                subprocess.Popen(["maya", "-file", f"{new_ver_path}/{seq}_{new_ver_num}_w001.mb"])
            elif houdini_user:
                self.mX
            elif nuke_user:
    ...
    def set_listwidget (self): # 리스트 위젯에 wip 파일 띄우기
        ...
        version_path = f"{wip_path}/scene/{version}"
    ...
    def double_clicked_item(self,item): # 더블클릭하면 실행하기

        _, part, _, version, wip_path, _ = self.get_tablewidget_data()

        scene_path = f"{wip_path}/scene/{version}"
